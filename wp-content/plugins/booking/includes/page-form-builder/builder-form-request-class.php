<?php /**
 * @version 1.0
 * @description WPBC_AJX__Builder_Booking_Form
 * @category  WPBC_AJX__Builder_Booking_Form Class
 * @author wpdevelop
 *
...
class WPBC_AJX__Builder_Booking_Form {
	...
	// <editor-fold     defaultstate="collapsed"                        desc=" ///  JS | CSS files | Tpl loading  /// "  >
		...
		public function init_load_css_js_tpl() {
			...
			if  (
				( strpos( $server_request_uri, 'page=wpbc-settings' ) !== false ) &&
				( strpos( $server_request_uri, '&tab=builder_booking_form' ) !== false )
			) {
	...
	public function js_load_files( $where_to_load ) {
		...
		public function enqueue_css_files( $where_to_load ) {
			...
			if ( ( wpbc_is_builder_booking_form_page() ) && ( is_admin() ) && ( in_array( $where_to_load, array( 'admin', 'both' ) ) ) ) {
				wp_enqueue_style( 'wpbc-ajx__builder_booking_form_page', trailingslashit( plugins_url( '', __FILE__ ) ) . '_out/builder-form-page.css', array(), WP_BK_VERSION_NUM );
			}
		}
...
//			, 'calendar__view__cell_height' 	=> array( 'validate' => 's',  	'default' => '48px' )		// '45px' || ''
	...
	// <editor-fold     defaultstate="collapsed"                        desc=" ///  Templates  /// "  >
		...
		public function hook__page_footer_tmpl( $page ){
			...
			if ( 'wpbc-ajx_booking_builder_booking_form'  === $page ) {		// from >>	do_action( 'wpbc_hook_settings_page_footer', 'wpbc-ajx_booking_availability' ); as availability_page.php in bottom  of content method
		...
		private function template__main_page_content() {
			?><script type="text/html" id="tmpl-wpbc_ajx__builder_booking_form_main_page_content">
					...
					return false;
						...
						?><div class="ui_element"><?php
							...
							?><select class="wpbc_ui_control wpbc_ui_select change_booking_resource_selectbox"
									  ...
									  onchange="javascript:wpbc_admin_show_message_processing( '' );wpbc_ajx__builder_booking_form__send_request_with_params( {
	...
	// <editor-fold     defaultstate="collapsed"                        desc=" ///  A J A X  /// "  >
		...
		public function define_ajax_hook(){
			...
			add_action( 'wp_ajax_'		     . 'WPBC_AJX__BUILDER_BOOKING_FORM', array( $this, 'ajax_' . 'WPBC_AJX__BUILDER_BOOKING_FORM' ) );	    // Admin & Client (logged in usres)
		 ...
		 */
		public function ajax_WPBC_AJX__BUILDER_BOOKING_FORM() {
			...
			$action_name    = 'wpbc_ajx__builder_booking_form_ajx' . '_wpbcnonce';
 ...
 */
if ( true ) {
	$ajx__builder_booking_form_loading = new WPBC_AJX__Builder_Booking_Form;
	$ajx__builder_booking_form_loading->init_load_css_js_tpl();
	$ajx__builder_booking_form_loading->define_ajax_hook();
}